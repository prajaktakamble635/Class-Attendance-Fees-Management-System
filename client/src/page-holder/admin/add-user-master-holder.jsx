...
export default function AddUserMasterHolder(props) {
  ...
  React.useEffect(() => {
    document.title = "Sproutedge Agro Add User Master";
  }, []);

  const closeDialog = () => {
    navigate("/admin/user-master", { replace: true });
  };
  ...
  const submitData = async () => {
    ...
    // Run your existing form validation first
    let hasError = validateFormData(formData, validationRules);

    // --- Additional mobile/email validation ---
    if (formData.mobile) {
    ...
    if (!hasError) {
      const { name, password, role, mobile, email } = formData;
      const data = { name, password, role, mobile, email };
