...
export default function EditAdmissionHolder() {
  ...
  }, [selectedBoard, selectedStandard, selectedMedium, boardSubjectConditions, initialLoad]);
  ...
  }, [installmentData, formData.paymentType, formData.totalFees, formData.paidFees, formData.discount, initialLoad]);
  ...
  const getStudentDetails = async () => {
    try {
      ...
      if (studentData?.boardCondition) {
        setSelectedBoard(studentData.boardCondition.boardName || "");
        setSelectedStandard(studentData.boardCondition.standard || "");
        setSelectedMedium(studentData.boardCondition.medium || "None");
      }
      ...
      if (studentData?.addons) {
        setSelectedAddons(studentData.addons.split(",").filter(a => a.trim() !== ""));
      } else {
        setSelectedAddons([]);
      }
  ...
  const availableAddons = useMemo(() => {
    ...
    const board = selectedCondition.boardName?.toLowerCase() || "";
    const std = selectedCondition.standard?.replace(/\D/g, '') || "";
    const medium = selectedCondition.medium?.toLowerCase() || "";
  ...
  const calculateTotalFee = useCallback(() => {
    if (!selectedCondition || !selectedCondition.boardName || !selectedCondition.standard) return formData.totalFees; // keep original if no condition
    ...
    return fee > 0 ? fee : formData.totalFees; // don't override with 0 if edit
  }, [selectedCondition, selectedSubjects, selectedSets, selectedAddons]);

  useEffect(() => {
    if (initialLoad || !isUserModified) return;
                ...
                <div className="mb-4 grid w-full grid-cols-1 gap-2 md:grid-cols-12">
                  <div className="col-span-12 md:col-span-4">
                    <Input
                      readOnly
                      label="Final Fees"
                      className="!bg-blue-gray-50 font-bold"
                        ...
                        <div className="mb-6 w-full">
                          ...
                          <ReactSelect
                            ...
                            styles={{
                              multiValue: (base, state) => {
                                return state.data.isFixed ? { ...base, backgroundColor: '#e2e8f0' } : base;
                              },
                              multiValueLabel: (base, state) => {
                                return state.data.isFixed
                                  ? { ...base, fontWeight: 'bold', color: '#475569', paddingRight: 6 }
                                  : base;
                              },
                          ...
                          <div className="mt-4 flex flex-wrap gap-2">
                            {selectedSubjects.map((s) => (
                              <span key={`badge-${s.id}`} className="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded">
