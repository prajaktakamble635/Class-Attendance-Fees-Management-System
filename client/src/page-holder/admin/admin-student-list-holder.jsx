...
export default function AdminStudentListHolder() {
  ...
  const [passwordResetStudId, setPasswordResetStudId] = useState(null);
  const [newParentPassword, setNewParentPassword] = useState("");

  const handleOpenPasswordReset = (id) => {
    setPasswordResetStudId(id);
    setNewParentPassword("");
    setIsPasswordModalOpen(true);
  };

  const handleClosePasswordReset = () => {
    setPasswordResetStudId(null);
    setNewParentPassword("");
  ...
  const handlePasswordSubmit = async () => {
    if (newParentPassword.length < 4) {
      return toast.warn("Password must be at least 4 characters long", { theme: theme == 'light' ? 'dark' : 'light' });
    }
    try {
      await axios.put(`${import.meta.env.VITE_API_URL}/api/superAdminApi/updateParentPassword/${passwordResetStudId}`, { newPassword: newParentPassword });
      toast.success("Parent password updated successfully", { theme: theme == 'light' ? 'dark' : 'light' });
      handleClosePasswordReset();
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed to update password", { theme: theme == 'dark' ? 'light' : 'dark' });
    }
  };
  ...
  return (
    <div className="animate-fade-in mb-8 mt-12 flex transform flex-col gap-12">
            ...
            <div className="flex flex-col justify-between md:flex-row">
              ...
              <div className="flex flex-col gap-2 md:flex-row">
                ...
                <div className="flex flex-row gap-2">
                  <Button
                    onClick={() => navigate("/superAdmin/admission-import")}
                  ...
                  <Button
                    onClick={() => navigate("/superAdmin/add-admission")}
                    className="inline-flex self-center"
                    variant="outlined"
                    color="white"
                    size="sm"
            ...
            <table className="w-full min-w-[640px] table-auto">
              <thead>
                <tr>
                  ...
                  <TableHeaderCell
                    key="fatherMobile"
                    columnName="fatherMobile"
                    text="Father_Mobile_No"
                    orderBy={tableProp.orderBy}
                    handleOrderBy={handleOrderBy}
                    isOrderByAvailable={true}
                    orderDirection={tableProp.orderDirection}
                  ...
                  <tr key={rowObj.id}>
                    <td className="items-center border-b border-blue-gray-50 px-2 py-2">
                      <div className="flex flex-row gap-3">
                        ...
                        <Tooltip content="reset parent password" className="p-1 text-xs">
                          <button
                            onClick={() => handleOpenPasswordReset(rowObj.id)}
                          >
                            <i className="fas fa-key text-base font-semibold text-orange-600"></i>
                          </button>
                        </Tooltip>
                        <Tooltip content="edit" className="p-1 text-xs">
                          <Link to={`/superAdmin/edit-student/${rowObj.id}`}>
                    ...
                    <td onClick={() => navigate(`/superAdmin/edit-student/${rowObj.id}`)} className="items-center hover:cursor-pointer border-b border-blue-gray-50 px-2 py-2">
                      <div className="flex flex-row gap-3">
                        <Typography className="text-xs font-semibold text-blue-gray-600">
                          {rowObj?.rollNo || ""}
                        </Typography>
                    ...
                    <TableCell text={rowObj?.fatherMobile || "--"} />
      ...
      <Dialog
        className="z-40"
        handler={handleClosePasswordReset}
      ...
      >
        ...
        >
          <div className="w-full flex flex-col gap-4">
            <Typography className="text-center">Enter a new password for the parent account associated with this student.</Typography>
            <Input
              label="New Password"
              type="password"
        ...
        <DialogFooter className="bg-gray-100 sticky bottom-0 z-10">
          <CancelButton onClick={handleClosePasswordReset} />
          <SubmitButton onClick={handlePasswordSubmit} title="Update Password" />
        </DialogFooter>
      </Dialog>
    </div>
  )
