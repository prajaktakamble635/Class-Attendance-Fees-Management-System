...
export default function AddAdmissionHolder() {
    ...
    if (totalCalculated !== total) {
      ...
      setInstalErrMsg(
        totalCalculated > total
          ? `Installments + Paid Fees exceed Total Fees (by ₹${(totalCalculated - total).toFixed(2)})`
          : `Installments + Paid Fees are less than Total Fees (by ₹${(total - totalCalculated).toFixed(2)})`
      );
    } else {
      setInstallmentErr(false);
      setInstalErrMsg("");
  ...
  const loadConditions = (inputValue, callback) => {
    const filtered = boardSubjectConditions.filter((condition) =>
