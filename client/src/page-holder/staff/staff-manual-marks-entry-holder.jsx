...
export default function AdminManualMarksEntry() {
    ...
    const handleSubmitMarks = async () => {
        ...
        try {
          ...
          navigate("/batchCoord/marks-entry");
        } catch (err) {
          console.error(err);
                ...
                <CardHeader color={sidenavColor} className="mb-4 mt-5 p-3">
                    <div className="flex flex-col justify-between md:flex-row">
                        ...
                        <Button
                          ...
                          color="white"
                          size="sm"
                          onClick={() => navigate('/batchCoord/marks-entry')}
                        >
                            Back
                        </Button>
                        </div>
                              ...
                              <div className="flex items-center gap-3">
                                {/* <Input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    // Allow empty input (backspacing to clear)
                                    if (val === "") {
                                      setMarksEntries((prev) =>
                                        prev.map((item, i) => (i === index ? { ...item, marks: "" } : item))
                                      );
                                      return;
                                    }
                                    ...
                                    // Prevent exceeding max marks
                                    if (numValue > Number(entry.maxMarks)) {
                                      toast.warning(`Entered marks exceed maximum marks for ${entry.label}`);
                                      numValue = Number(entry.maxMarks);
                                    }
