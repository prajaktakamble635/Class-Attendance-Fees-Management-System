...
import Update from "@/page-sections/profile/update";
import { useNavigate } from "react-router-dom";
import dayjs from "dayjs";
import { useUser } from "@/context/user";

export default function UpdatePreview(props) {
  const {
    open,
    handleClose,
    formData,
  ...
  const submitData = async () => {
    ...
    try {
      ...
      await axios.post(`${import.meta.env.VITE_API_URL}/api/superAdminApi/updateStudentAdmission`, data);
      closeDialog()
      toast.success("Student Details Updated")
    } catch (err) {
      const errmsg = err?.response?.data?.message || "Internal Server Error: Failed to admin student, please contact system administrator.";
      toast.error(errmsg)
    } finally {
      setIsUpdating(false)
    }
  ...
  const closeDialog = () => {
    ...
    handleClose()
    if (user?.userRole == 1) {
      navigate("/superAdmin/student-list")
    } else if (user?.userRole == 3) {
      navigate("/counsellor/student-list")
    } else if (user?.userRole == 4) {
      navigate("/hod/student-list")
    } else { null }
  ...
  return (
    ...
    >
        ...
        <div className="w-full">
          ...
          <div className="my-4 w-full bg-blue-600 py-2 rounded-lg shadow-sm">
            <Typography className="font-semibold text-white text-center">
              Gurukul TEST SERIES - ADMISSION  (Updated)
            </Typography>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4 bg-white rounded-xl shadow-sm">
            ...
            <div className="md:col-span-2 lg:col-span-3">
              ...
              <div className="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50 text-gray-800 font-medium leading-relaxed">
                {formData?.address || "123 Main Street, Springfield, USA"}
              </div>
            </div>
          ...
          {selectedSubjects && selectedSubjects.length > 0 ? (
            <div className="my-2 w-full flex flex-wrap ms-4">
              {selectedSubjects.map((item, idx) => (
                <div key={idx} className="px-3 py-1 bg-gray-200 border border-gray-300 m-2 rounded-lg">
                  {item.code}
                </div>
              ))}
            </div>
      ...
      <DialogFooter className="bg-gray-100 sticky bottom-0 z-10">
        ...
        <UpdateButton disabled={isUpdating} onClick={submitData} />
      </DialogFooter>
    </Dialog>
  );
}
