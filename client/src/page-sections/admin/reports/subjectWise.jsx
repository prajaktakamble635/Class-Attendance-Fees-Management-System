...
export default function SubjectWiseReport(props) {
  ...
  const fetchSubjects = async (id) => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/superAdminApi/subjectsByCondition?examSessionId=${id}`,
      );
  ...
  const fetchStudents = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/superAdminApi/getStudentDataForReports?conditionId=${conditionValue?.value}&setId=${examSessionValue?.setIdFk}`);
  ...
  const loadStudents = async (inputValue) => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/superAdminApi/getStudentDataReportsForSelect?conditionId=${conditionValue?.value}&setId=${examSessionValue?.setIdFk}&word=${inputValue}`
      );
  ...
  const handleFetchAllStudents = async (currentPage = 1, perPage = 50) => {
    ...
    let subjectIdsData = subjectValue && subjectValue.length > 0 ? subjectValue.map(item => item.value) : [];
    let studentIds = studentValue && studentValue.length > 0 ? studentValue.map(item => item.value) : []
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/api/superAdminApi/getStudentMarksReport`, {
        currentPage, perPage, setId: examSessionValue?.setIdFk, conditionId: conditionValue?.value,  examSessionId: examSessionValue?.value,
        subjectIdData: subjectIdsData || [],
        student: studentIds || []
      })
  ...
  const prepareForDownload = () => {
    ...
    axios
      .post(
        ...
        }/api/superAdminApi/getStudentMarksReport`,
        { currentPage: 1, perPage: 1000, setId: examSessionValue?.setIdFk, conditionId: conditionValue?.value, subjectIdData: subjectValue && subjectValue.length > 0 ? subjectValue.map(item => item.value) : [], student: studentValue && studentValue.length > 0 ? studentValue.map((s) => s.value) : [], examSessionId: examSessionValue?.value },
      )
      .then(async (response) => {
  ...
  const processCSVData = (tempData) => {
    ...
    csvDataTemp.push(["", "", "Subject Wise Marks Report"]);
    csvDataTemp.push([]);
    ...
    tempData.forEach((student, index) => {
      ...
      subjectHeaders.forEach(subject => {
        row.push(student[subject] === "NA" ? "NA" : student[subject]);
      });
  ...
  const downloadExcelFile = (data) => {
    ...
    const fileName = `subject-wise-marks-(${conditionValue?.label})-${moment().format("YYYY-MM-DD-HH-mm-ss")}.xlsx`;
              ...
              <table className="w-full min-w-[640px] table-auto">
                ...
                <tbody>
                  {tableData && tableData.length === 0 ? (
                    ...
                    <tr key={rowObj.id}>
                      <td className="items-center border-b border-blue-gray-50 px-2 py-2">{index + 1}</td>
